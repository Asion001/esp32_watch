                               ...
                               lv_dir_t direction)
{
  ...
  // Following LVGL smartwatch demo pattern:
  // - Parent screen captures gestures (GESTURE_BUBBLE cleared)
  // - Children handle orthogonal scrolling via explicit scroll direction
  // - This allows vertical scroll while horizontal swipe triggers back navigation
  lv_obj_clear_flag(screen, LV_OBJ_FLAG_GESTURE_BUBBLE);

  // Disable horizontal scroll snap to prevent accidental triggering during vertical scroll
  lv_obj_set_scroll_snap_x(screen, LV_SCROLL_SNAP_NONE);
}
