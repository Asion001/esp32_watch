        ...
        switch (event_id) {
            ...
            case WIFI_EVENT_STA_DISCONNECTED:
                ...
                if (wifi_mgr.retry_count < MAX_RETRY_ATTEMPTS) {
                    ...
                    ESP_LOGE(TAG, "Connection failed after %d attempts", MAX_RETRY_ATTEMPTS);
            ...
            case WIFI_EVENT_SCAN_DONE:
                ...
                // Fetch scan results
                wifi_mgr.scan_count = 20;  // Max 20 APs
                wifi_ap_record_t ap_records[20];
                esp_wifi_scan_get_ap_records(&wifi_mgr.scan_count, ap_records);
                
                // Convert to our format
                for (int i = 0; i < wifi_mgr.scan_count; i++) {
                    strncpy(wifi_mgr.scan_results[i].ssid, (char*)ap_records[i].ssid, 32);
                    wifi_mgr.scan_results[i].ssid[32] = '\0';
...
esp_err_t wifi_manager_get_scan_results(wifi_ap_info_t *ap_list, uint16_t *ap_count)
{
    ...
    uint16_t count = (*ap_count < wifi_mgr.scan_count) ? *ap_count : wifi_mgr.scan_count;
...
esp_err_t wifi_manager_connect(const char *ssid, const char *password, bool save_credentials)
{
    ...
    if (password) {
        ...
        if (pass_len > 0 && (pass_len < 8 || pass_len > 64)) {
            ESP_LOGE(TAG, "Invalid password length: %d (must be 8-64 chars)", pass_len);
            return ESP_ERR_INVALID_ARG;
        }
    }
    ...
    // Clear event bits
    xEventGroupClearBits(wifi_mgr.event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT);
...
esp_err_t wifi_manager_auto_connect(void)
{
    ...
    esp_err_t ret = settings_get_string(SETTING_KEY_WIFI_SSID, "", ssid, sizeof(ssid));
    if (ret != ESP_OK || strlen(ssid) == 0) {
        ESP_LOGE(TAG, "Failed to load SSID");
        return ESP_FAIL;
    }
    
    settings_get_string(SETTING_KEY_WIFI_PASSWORD, "", password, sizeof(password));
