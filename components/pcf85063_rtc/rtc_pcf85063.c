...
esp_err_t rtc_init(i2c_master_bus_handle_t i2c_bus) {
  ...
  if (get_build_time(&build_time) == 0) {
    ESP_LOGI(TAG, "Setting RTC to: %04d-%02d-%02d %02d:%02d:%02d",
             build_time.tm_year + 1900, build_time.tm_mon + 1,
...
esp_err_t rtc_read_time(struct tm *time) {
  ...
  esp_err_t ret =
      i2c_master_transmit_receive(rtc_dev, (uint8_t[]){PCF85063_REG_SEC}, 1,
                                  data, 7, 1000 / portTICK_PERIOD_MS);
...
esp_err_t rtc_write_time(const struct tm *time) {
  ...
  esp_err_t ret =
      i2c_master_transmit(rtc_dev, data, 8, 1000 / portTICK_PERIOD_MS);
  if (ret != ESP_OK) {
...
bool rtc_is_valid(void) {
  ...
  if (time.tm_hour > 23 || time.tm_min > 59 || time.tm_sec > 59) {
    ESP_LOGW(TAG, "Invalid time: %02d:%02d:%02d", time.tm_hour, time.tm_min,
             time.tm_sec);
    return false;
  }
