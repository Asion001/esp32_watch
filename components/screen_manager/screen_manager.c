...
lv_obj_t *screen_manager_create(const screen_config_t *config)
{
    ...
    // - Vertical screens slide UP to open (MOVE_BOTTOM), so swipe DOWN to close (DIR_TOP)
    // - Horizontal screens slide RIGHT to open (MOVE_LEFT), so swipe LEFT to close (DIR_LEFT)
    if (config->anim_type != SCREEN_ANIM_NONE && config->hide_callback)
    {
        lv_dir_t gesture_dir = (config->anim_type == SCREEN_ANIM_VERTICAL) ? LV_DIR_TOP : LV_DIR_LEFT;
...
esp_err_t screen_manager_go_back(void)
{
    ...
    {
        screen_nav_go_back_horizontal(current, previous); // Horizontal (left-to-right)
    }
    else
    {
    ...
    s_manager.previous_screen = NULL; // Clear previous (simple two-level navigation)
    s_manager.current_anim = SCREEN_ANIM_NONE;
    s_manager.current_hide_cb = NULL;
...
bool screen_manager_is_root(lv_obj_t *screen)
{
    ...
    return (s_manager.current_screen == screen && s_manager.previous_screen == NULL);
}
