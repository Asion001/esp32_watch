...
esp_err_t pmu_get_battery_percent(uint8_t *percent) {
    ...
    uint16_t voltage_mv;
    esp_err_t ret = pmu_get_battery_voltage(&voltage_mv);
    if (ret != ESP_OK) {
        return ret;
    }
    ...
    if (voltage_mv <= VBAT_MIN_MV) {
        ...
        calculated_percent = (voltage_mv - VBAT_MIN_MV) * 50 / (VBAT_NOMINAL_MV - VBAT_MIN_MV);
    } else {
        // 3.7V to 4.2V range (50% to 100%)
        calculated_percent = 50 + (voltage_mv - VBAT_NOMINAL_MV) * 50 / (VBAT_MAX_MV - VBAT_NOMINAL_MV);
    }
    
    // Clamp to 0-100 range
    *percent = (calculated_percent < 0) ? 0 : (calculated_percent > 100) ? 100 : calculated_percent;
...
esp_err_t pmu_is_charging(bool *is_charging) {
    ...
    esp_err_t ret = i2c_master_transmit_receive(
        pmu_dev,
        (uint8_t[]){AXP2101_REG_CHG_STATUS}, 1,
        &status, 1,
        1000 / portTICK_PERIOD_MS
    );
