...
esp_err_t settings_get_int(const char *key, int32_t default_value,
  ...
  if (ret == ESP_ERR_NVS_NOT_FOUND) {
    *value = default_value;
    ESP_LOGD(TAG, "Key '%s' not found, using default: %d", key,
             (int)default_value);
  } else if (ret != ESP_OK) {
...
esp_err_t settings_get_uint(const char *key, uint32_t default_value,
  ...
  if (ret == ESP_ERR_NVS_NOT_FOUND) {
    *value = default_value;
    ESP_LOGD(TAG, "Key '%s' not found, using default: %u", key,
...
esp_err_t settings_get_string(const char *key, const char *default_value,
  ...
  if (ret == ESP_ERR_NVS_NOT_FOUND) {
    if (default_value) {
      strncpy(value, default_value, max_len - 1);
      value[max_len - 1] = '\0';
      ESP_LOGD(TAG, "Key '%s' not found, using default: %s", key,
               default_value);
    } else {
      value[0] = '\0';
    }
  ...
  if (required_size > max_len) {
    ESP_LOGW(TAG, "Buffer too small for '%s' (need %u, have %u)", key,
             (unsigned)required_size, (unsigned)max_len);
    nvs_close(nvs_handle);
